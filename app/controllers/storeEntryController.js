...
exports.add_new_store_entry = function (req, res) {
    ...
    new_store_entry.starting_amount = request.drawer_amount;
    let new_store_amount = (parseInt(request.store_amount) + parseInt(request.bank_deposit));
    let new_drawer_amount = (parseInt(request.remain_amount));

    StoreEntry.addNewStoreEntry(supply_details, expense_details, new_store_entry, new_store_amount, new_drawer_amount, function (err, storeEntry) {
        if (err) {
            res.send(err);
        } else {
            res.sendStatus(200);
